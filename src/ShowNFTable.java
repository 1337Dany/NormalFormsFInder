...
public class ShowNFTable {
    ...
    private void showResult() {
        ...
        String relation = getArrayDataAsLine(Relation.getRelation());

        System.out.printf("%-" + columnWidth + "s%-" + columnWidth + "s%-" + columnWidth + "s%-" + columnWidth + "s%n"
                        + columnWidth + "s%n",
                getArrayDataAsLine(Relation.getRelation()),
        ...
        for (ArrayList<String> key : Dependency.getDependencies().keySet()) {
            ...
            String value5 = NFFinder.isPartialDependancy(key)? "T" : "F";
                    ...
                    System.out.printf("%" + (columnWidth + value1.length()) / 2 + "s%" + (columnWidth + value2.length()) / 2 + "s%"
                            + (columnWidth + value3.length()) / 2 + "s%" + (columnWidth + value4.length()) / 2 + "s%n"
